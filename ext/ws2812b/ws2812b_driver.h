...
typedef struct
{
		...
		volatile uint16_t length; // buffer size
		volatile uint8_t sector_size; // buffer size per one SPI transaction
} spi_buffer_t;
...
void spi0_event_handler(nrf_drv_spi_evt_t const * event, void * p_context);
void spi1_event_handler(nrf_drv_spi_evt_t const * event, void * p_context);
